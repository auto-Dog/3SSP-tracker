...
#define vRatio 10.2			//ratio of travel steps:distance
#define DeteceterOhm 0.01
#define tauLimitL -0.8726	//rad
...
bool motorMove::changeheight(double inputHeight1,double inputHeight2){	
	...
	counter1 = 0;counter2 = 0;					//zerolize coder counter
...
bool motorMove::motorSetup(double oriheight1,double oriheight2){		
	...
	delay(100000);				//THIS VALUE CHANGE DUE TO THE FACT
	digitalWrite(6,0);
	digitalWrite(7,0);
	digitalWrite(8,0);
	digitalWrite(9,0);
...
void motorMove:: movetoflat(double val){				//go to 400mm height
	if(val<5||val>790)	return;
	changeheight(val,val);
}

void motorMove:: movetodir(double tau,double heightAngle){	//for initial tracking, input obj alpha,obj beta
  ...
  heightAngle = heightAngle/180.0*3.14159;
	...
	tmpheight1 = -0.5*triangleR*(cos(currentBeta)/sin(currentBeta))*(3*abs(cos(currentAlpha))+1.732*sin(currentAlpha))+400;
	tmpheight2 = -0.5*triangleR*(cos(currentBeta)/sin(currentBeta))*(3*abs(cos(currentAlpha))-1.732*sin(currentAlpha))+400;	
